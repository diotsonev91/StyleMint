...
const buildUpdateFormData = (data: UpdatePackDto): FormData => {
    ...
    if (data.samplesToRemove && Array.isArray(data.samplesToRemove)) {
        ...
        console.log('🗑️ updateApi - Building FormData with samplesToAdd:', data.samplesToAdd);
...
export const updateApi = {
    ...
    /**
     * ✅ NEW: Unbind sample from pack
     * DELETE /api/v1/audio/samples/{sampleId}/pack/{packId}
     *
     * This is called during pack editing when user removes a sample from the pack.
     * It unbinds the sample from the pack without deleting the sample itself.
     */
    unbindSampleFromPack: async (sampleId: string, packId: string): Promise<ApiResponse> => {
        try {
            console.log(`🗑️ API: Unbinding sample ${sampleId} from pack ${packId}`);

            const response = await API.delete(`/audio/samples/${sampleId}/pack/${packId}`);

            console.log('✅ API: Sample unbound successfully:', response.data);
            ...
            console.error('❌ API: Failed to unbind sample:', error);

            return {
                success: false,
                error: error.response?.data?.message || error.message || 'Failed to unbind sample from pack'
            };
        }
    }
};